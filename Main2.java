...
public class Main2 {
	...
	public static void main(String[] args) {
		...
		for(int ingresos = 0;ingresos < 15;ingresos++) {			//O(n^2) = Sumatoria de c1 hasta c17 desde, que queda como O(n^2)
			System.out.println("Bienvenido/a al banco 9 de Oro!");		//O(1) --> c1
			try {
				System.out.println("Ingrese su ID (8 carácteres): ");	//O(1) --> c2
				id = input.nextInt();					//O(1) --> c3
				String id_s = Integer.toString(id);			//O(1) --> c4
				int long_id = id_s.length();				//O(1) --> c5
				if(long_id < 8|| long_id > 8) {				//O(1) --> c6
					System.out.println("Error. El ID debe contener 8 carácteres.\nIngrese su ID: "); //O(1) --> c7
					id = input.nextInt();		//O(1) --> c8
				}
				System.out.println("Ingrese tipo de cliente:\n1.Particular no cliente\n2.Particular cliente\n3.Empresa"); //O(1) --> c9
				cliente = input.nextInt();	//O(1) --> c10																			  
				if(cliente<1 || cliente>3) {																			  
					System.out.println("Error. Ingrese tipo de cliente:\n1.Particular no cliente\n2.Particular cliente\n3.Empresa"); //O(1) --> c11
					cliente = input.nextInt();					//O(1) --> c12																 
				}		
				cola.AcolarPrioridad(id,cliente); 							 //O(n) --> c13
			} catch(InputMismatchException e) {								 //O(1) --> c14
				System.out.println("Error. Ingrese un valor válido de ID."); //O(1) --> c15
				ingresos--; 												 //O(1) --> c16
				input.next(); 												//O(1) --> c17									 //O(1) --> c17
			}
